class_name UVPositionCalculator
extends Node
...
func _init(_mesh_instance: MeshInstance3D) -> void:
	set_mesh(_mesh_instance)
...
func get_face(point: Vector3, normal: Vector3, epsilon := 0.1) -> Array:
	var matches = []
	for idx in range(_face_count):
...
func get_uv_coords(point: Vector3, normal: Vector3, transform := true):
